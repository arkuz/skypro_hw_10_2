...
@app.route("/")
def index():
    return get_formatted_candidates(candidates)


@app.route("/candidates/<name>")
def candidate_by_name(name):
    candidate = get_candidate_by_name(candidates, name)
...
def candidates_by_skill(skill):
    return get_formatted_candidates(get_candidates_by_skill(candidates, skill))
